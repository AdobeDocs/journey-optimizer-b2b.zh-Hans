...
要执行操作，如发送电子邮件、更改得分或分配给购买组，您可以在历程中添加&#x200B;_[!UICONTROL 执行操作]_&#x200B;节点。 操作通常是您希望因某种类型的触发器（例如事件或上一个操作）而发生的操作。

![视频](../../assets/do-not-localize/icon-video.svg){width="30"} [观看概述视频](#overview-video)

## 帐户操作 {#account-actions}

在帐户历程中，当您想要将更改应用于节点路径上帐户的所有成员时，请使用对帐户的操作。
...
### 激活目标或从目标取消激活 {#activate-deactivate-destination}

使用&#x200B;_激活到目标_&#x200B;操作，直接从历程激活到Experience Platform目标的帐户。 此操作可让您将符合条件的帐户（基于购买组过滤器、参与度得分和其他标准）推送到受支持目标的匹配受众。

您可以稍后在同一历程中将此操作与&#x200B;_从目标取消激活_&#x200B;操作配对，以在帐户转换或不再符合条件后停止定位帐户。
...
1. 在&#x200B;**[!UICONTROL 虚拟受众]**&#x200B;下，单击&#x200B;**[!UICONTROL 选择现有虚拟受众]**，然后选择要从目标中删除其帐户的虚拟受众。

1. 在&#x200B;**[!UICONTROL 从目标]**&#x200B;取消激活&#x200B;**[!UICONTROL 选择目标]**&#x200B;并选择要从中删除帐户的目标。

   配置&#x200B;_从目标_&#x200B;节点取消激活后，将显示所选虚拟受众和目标。

>[!ENDSHADEBOX]

## 人员操作 {#people-actions}

在帐户或人员历程中，当您想要将更改应用于节点路径上的所有人员时，对人员使用操作。 对于帐户历程，您可以在&#x200B;_按人员拆分路径_&#x200B;或&#x200B;_按帐户拆分路径_&#x200B;节点中使用此节点类型。
...
| | [!UICONTROL 发送电子邮件] | <li>帐户历程 <li>人员历程 | <li>创建电子邮件 <li>发送时间优化（可选，仅限人员历程） |
| | [!UICONTROL 发送短信] | <li>帐户历程 | <li>创建短信 |
| | [!UICONTROL 发送WhatsApp] | <li>帐户历程 | <li>创建WhatsApp消息 |
| | [!UICONTROL 更新人员配置文件] | <li>帐户历程 <li>人员历程 | <li>选择人员属性 <li>设置新值 |
| [Marketo Engage](#marketo-engage-actions) | [!UICONTROL 添加到Marketo请求营销活动] | <li>帐户历程 <li>人员历程 | <li>选择 Marketo Engage 工作区 <li>选择请求营销活动 |
| | [!UICONTROL 添加到Marketo列表] | <li>帐户历程 <li>人员历程 | <li>外部Marketo连接的名称 <li>列表名称 |
| | [!UICONTROL 从Marketo列表中删除] | <li>帐户历程 <li>人员历程 | <li>外部Marketo连接的名称 <li>列表名称 |

>[!NOTE]
>
>Marketo Engage中的&#x200B;_[!UICONTROL 更改人员分区]_&#x200B;和&#x200B;_[!UICONTROL 更改得分]_&#x200B;操作在2025.10版本中已弃用，在Journey Optimizer B2B edition中不再可用。<br/>
>
>2025.10版本弃用&#x200B;_[!UICONTROL 更改数据值]_&#x200B;操作。 在当前Journey Optimizer B2B edition版本中，它已被替换为&#x200B;_[!UICONTROL 更新人员配置文件]_。
...
1. （_仅限帐户历程_）在右侧的节点属性中，为操作上下文选择&#x200B;**[!UICONTROL 人员]**。
...
+++[!UICONTROL 将人员添加到（其他）历程]

使用此操作可将人员配置文件添加到实时人员历程。 当人员进入操作节点时，系统会将其添加为指定人员历程的受众成员。

使用&#x200B;**[!UICONTROL 选择实时人员历程]**&#x200B;选择器指定要添加人员配置文件的人员历程。 您可以在字段中输入文本以筛选列表。

![执行操作 — 将人员添加到（其他）历程](./assets/node-action-add-to-person-journey.png){width="300"}

+++

+++[!UICONTROL 添加到外部客户受众]

使用此操作可将人员推送至外部受众，该受众可通过付费媒体渠道激活，以进一步定位购买群组的成员。 此操作通过Real-Time CDP B2B edition执行。
...
从2025.10版本开始，您还可以通过在[!DNL Experience Platform]中创建的外部受众（如[!DNL Adobe Target]目标）进行编排。 有关此受众集成的更多详细信息，请参阅[Adobe Target外部受众](../audiences/target-external-audience.md)。
...
+++[!UICONTROL 个性化Web体验] (Beta)

使用此操作直接[在您的网站上创建个性化体验](../content/web-experiences.md)。 Web渠道功能提供了一个灵活的工具包，您可以使用它来增强对定制Web内容的参与。

![执行操作 — 个性化Web体验](./assets/node-action-person-personalize-web-experience.png){width="300"}
...
使用此操作可从人员历程中删除人员配置文件。 当人员进入操作节点时，系统将删除他们作为指定人员历程的受众成员。

使用&#x200B;**[!UICONTROL 选择人员历程]**&#x200B;选择器指定要从中删除人员配置文件的人员历程。 您可以在字段中输入文本以筛选列表。

![执行操作 — 从历程中删除人员](./assets/node-action-remove-from-person-journey.png){width="300"}

+++

+++[!UICONTROL 发送电子邮件]

使用此操作发送电子邮件。 在您[为节点](../content/add-email.md#add-an-email-action-node-in-a-journey)创建电子邮件后，您可以在电子邮件设计空间设计、个性化和预览电子邮件（请参阅[电子邮件创作](../content/email-authoring.md)）。 您还可以从Marketo Engage[&#128279;](https://experienceleague.adobe.com/zh-hans/docs/marketo/using/product-docs/email-marketing/general/creating-an-email/create-an-email){target="_blank"}发送电子邮件。 选择Marketo Engage工作区，然后选择要发送的电子邮件。

![执行操作 — 发送电子邮件](./assets/node-action-send-email-from-marketo.png){width="300"}

对于人员历程，使用[发送时间优化](../content/email-send-time-optimization.md)，通过预测每个用户档案最有可能参与的时间，使电子邮件投放时间个性化。

>[!NOTE]
>
>您可以在帐户历程中使用电子邮件重复数据删除，以确保同一电子邮件不会在历程中多次发送到同一电子邮件地址。 有关详细信息，请参阅[电子邮件重复数据删除](../content/email-deduplication.md)。
...
![执行操作 — 发送WhatsApp](./assets/node-action-send-whatsapp.png){width="300"}
...
>
>管理员可以通过更新&#x200B;_[!UICONTROL XDM配置]_ > [!UICONTROL 标准类]中的字段来配置XDM个人配置文件的可用属性。 有关详细信息，请参阅[标准架构](../admin/xdm-field-management.md#standard-schemas)。

+++

### Marketo Engage操作

[!DNL Marketo Engage]个基于人员的操作旨在协调您在[!DNL Journey Optimizer B2B Edition]中的Account-Based Marketing编排与您在Marketo Engage中基于潜在客户的营销工作。 使用这些操作可编排列表成员资格并请求营销活动。

>[!NOTE]
>
>Marketo Engage操作需要配置与一个或多个外部Marketo Engage实例的集成。 有关此配置的详细信息，请参阅&#x200B;[_激活Marketo Engage连接以支持操作_](../admin/marketo-actions-connect.md)。
