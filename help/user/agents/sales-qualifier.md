...
* _此电子邮件的目标是开始对话并建立信任。_ 起草一封有120个字的电子邮件，其语气具有咨询性和同理心。 请务必避免过于熟悉或过于熟悉的销售方式，不要使用“希望您一切顺利”、“登记入住”或“请”等词语。_
...
展开电子邮件接触点以读取其提示。 本说明在编写每个潜在客户的电子邮件（包括语气、时长、焦点和call to action）时指导AI。
...
* **在每个潜在客户生成之前完成接触点提示。****批量生成之后，通常一次只对一个潜在客户进行更改。
* **使用推理作为质量检查。** 如果强调了错误的信号（或明显缺少信号），请编辑电子邮件或重新访问接触点提示并重新生成频率。
* **将编辑工具与更改相匹配。****直接编辑文字和语调；**[!UICONTROL 使用AI生成]**&#x200B;以重新构建或重新设置框架。
* **仅批准您审查的内容。****在注册之前，展开接触点，阅读内容并在需要时进行优化。
