...
在对话框中，输入有用的名称和描述，然后单击&#x200B;**[!UICONTROL 创建]**。

将打开&#x200B;_[!UICONTROL 设计您的模板]_&#x200B;页面，该页面提供了多个用于创建模板的选项：从头开始设计、导入HTML或选择设计模板。

### 从头开始设计

使用电子邮件设计器定义电子邮件内容的结构。 通过通过简单的拖放操作添加和移动结构元素，您可以在几秒钟内设计可重用电子邮件内容的形状。

1. 从&#x200B;_[!UICONTROL 设计您的模板]_&#x200B;主页中，选择&#x200B;**[!UICONTROL 从头开始设计]**&#x200B;选项。

1. 将组件拖放到画布中以定义电子邮件的结构布局，开始设计内容。

   可用的设计工具等效于用于电子邮件创作的工具。 不同之处在于，此内容随后另存为模板，可在帐户历程中的多个发送电子邮件节点中重复使用。
...
此时将显示在当前沙盒中创建的所有电子邮件模板的列表。 您可以按名称、上次修改时间和上次创建时间对它们进行排序。

从列表中选择所需的模板。

选择后，这将显示模板的预览。 在预览模式下，您可以使用左右箭头在某个类别（示例或已保存，具体取决于您的选择）的所有模板之间导航。

当显示与您想要使用的内容匹配时，单击预览窗口右上角的使用此模板。

此操作会将内容复制到可视内容设计器中，您可以在其中根据需要编辑内容。

>[!TAB 示例模板]

Adobe Journey Optimizer B2B Edition提供一系列现成的电子邮件模板，可用于创建电子邮件和电子邮件模板。
...
选择&#x200B;_n：n列_&#x200B;组件以定义您选择的列数（3到10之间）。 通过移动底部的箭头来定义每列的宽度。

>[!NOTE]
>
>每个列大小不能低于结构组件总宽度的10%。 您只删除空列。
...
保存电子邮件后，该电子邮件会显示在片段详细信息页面>使用者部分。 添加到电子邮件模板的片段在模板中不可编辑 — 内容由源片段定义。

## 添加资源

在可视内容编辑器中，_Assets_&#x200B;图标显示在左侧。 以下示例概述了将资源添加到模板内容的步骤。

1. 要打开资源库，请单击&#x200B;_Assets_&#x200B;图标。

   在资源选择器中，您可以直接选择存储在Assets库中的资源。

1. 双击包含所需资产的文件夹。

1. 将一个或多个图像资产拖放到结构组件中。

## 预览和编辑URL

1. 单击左侧的&#x200B;_[!UICONTROL 链接]_&#x200B;选项卡以显示要跟踪的内容的所有URL。

1. 如果需要，请修改&#x200B;_跟踪类型_&#x200B;或&#x200B;_标签_&#x200B;并添加&#x200B;_标签_。
...
单击电子邮件模板的名称，以打开电子邮件模板详细信息页面。

对电子邮件模板执行快速操作，如右上角的&#x200B;_复制_&#x200B;和&#x200B;_删除_。

您还可以通过单击“警报”按钮来查看“警报”（电子邮件模板的错误和警告）。 虽然这些警报不禁止在电子邮件创建中使用电子邮件模板，但此信息让团队中的营销人员能够查看哪些内容可能不起作用，以及在将其用于投放之前所需的更新。
...
从电子邮件模板列表页面（“内容管理”>“电子邮件模板”）中，通过选中左侧的复选框一次选择多个模板。 选择多个模板时，底部会显示一个横幅。
...
1. 要使用自定义电子邮件模板，请选择&#x200B;**[!UICONTROL 保存的模板]**&#x200B;选项卡。
   ...
   此选项卡显示在沙盒中创建的所有电子邮件模板的列表。 您可以按名称、上次修改时间和上次创建时间对它们进行排序。
