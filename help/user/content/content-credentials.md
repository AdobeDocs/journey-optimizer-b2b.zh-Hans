...
description: 了解Adobe Journey Optimizer B2B edition如何将Content Credentials自动应用于使用创作AI工具生成或编辑的图像，以及这对于您的内容意味着什么。
...
Content Credentials会与图像文件一起保留。 使用创作AI生成或编辑的图像上传到[!DNL Adobe Journey Optimizer B2B Edition]或从导出时，其Content Credentials将保留。
...
## 影响Content Credentials的操作 {#cc-workflows}

>[!INFO]
>
>围绕创新型人工智能透明度的新法律正在出现，Adobe正在努力满足跨司法辖区的适用要求。 Content Credentials是Adobe用于满足这些法律要求的来源工具。

在[!DNL Journey Optimizer B2B Edition]中使用创作AI工具生成或编辑图像时，Content Credentials会自动附加到该图像，您无需执行任何操作。

### 生成图像 {#generate}

**_Example:_**&#x200B;通过描述所需视觉效果的文本提示为电子邮件生成横幅图像。 Content Credentials会附加到生成的图像。

在从文本提示、参考图像创建新图像或生成类似图像时，始终附加Content Credentials。

### 裁切图像 {#crop}

**_示例:_**

* 裁切生成的横幅图像以适合网页。 Content Credentials会通过裁切进行保留。
* 使用上传的照片库作为电子邮件背景，并裁切它以适合屏幕。 如果库存照片中不包含可产生人工智能的信息，则不会创建Content Credentials。

对图像文件进行调整时（例如将其裁剪为请求的尺寸），仅当源图像已具有图像文件时，才会保留其Content Credentials。 裁切会重新创建图像的像素，通常会删除该Content Credential，因此AI Assistant在裁切之前会从源图像读取该像素，然后重新创建该像素，并将其重新附加到裁切的结果。 裁剪本身不会添加新的创新型人工智能操作；而是保留现有操作。

### 添加文本叠加

**_Example:_**&#x200B;在登陆页面生成的背景图像上生成促销标题作为文本叠加。 来自背景图像的Content Credentials将被保留。

在背景图像上渲染生成的文本时，仅当背景图像已具有Content Credentials时，才会将Content Credentials附加到生成的图像中。 渲染叠加会生成一个新图像，因此图像编辑工具从背景中读取Content Credentials并将它们重新附加到结果中。 叠加步骤不会添加新的创作AI操作。

### 叠加图像

**_示例:_**

* 通过将生成的产品图像与生成的背景相结合，创建电子邮件标题。 这个结果带有Content Credentials，它反映了两个创新型人工智能来源。
* 将两张上传的品牌照片合并到一个拼贴图像中。 由于源图像都没有生成性人工智能操作，因此不会创建Content Credentials。

当您将两个或更多图像组合在一起，并且任何源图像具有Content Credentials时，组合的图像会保留它们，并合并到单个Content Credentials元数据元素中。 合成会从源中生成一个新图像，该图像通常会删除这些Content Credentials。 但是，图像编辑工具在合成之前读取每个元素，然后构建单个组合Content Credentials元素，其中列出每个有助于生成人工智能操作的源。
