...
通过[!DNL Journey Optimizer B2B Edition]设计工具，营销人员可以为&#x200B;**_电子邮件_**&#x200B;和&#x200B;**_登陆页面_**&#x200B;生成内容。 根据Web内容无障碍准则(WCAG) 2.1 （级别AA），使用这些工具遵守此指令。
...
在[!DNL Journey Optimizer B2B Edition]中，可以使用可视设计空间&#x200B;**[!UICONTROL 样式]**&#x200B;窗格中的样式参数和属性进一步细化内容中不同元素的大小和间距。

例如，您可以更新背景，或更改边距、填充和对齐方式，以提高内容的可视访问性。

![样式窗格，带有背景、边距、填充和对齐设置](assets/accessible-styles.png){width="700" zoomable="yes"}

通过[!DNL Journey Optimizer B2B Edition]可视化设计工具，您可以预览和优化不同设备和屏幕大小的设计。 您可以随时&#x200B;**[!UICONTROL 切换到实时视图]**，以查看内容在各种设备大小上的呈现方式。

![显示桌面、平板电脑和移动设备预览选项的实时视图切换](assets/accessible-live-view.png){width="700" zoomable="yes"}

>[!CAUTION]
>
>实时视图是一个通用预览，用于比较呈现在各种设备大小中的外观。 最终渲染可能会因收件人的电子邮件客户端或Web浏览器而异。

## 对图像使用替换文本 {#alt-text}

使用&#x200B;**[!UICONTROL 图像]**&#x200B;组件为图像提供替换文本。 有关图像组件设置的详细信息，请参阅&#x200B;[_内容组件_](content-components.md#image)
...
在可视设计空间中，使用相关的[结构](structure-components.md)和[内容](content-components.md)组件以清晰、逻辑和简洁的方式组织您的内容，让所有人都可以访问。 使用结构继承最佳实践在&#x200B;**[!UICONTROL 样式]**&#x200B;窗格中设置选项。

![电子邮件设计空间，显示有条理布局的结构和内容组件](assets/accessible-components.png){width="700" zoomable="yes"}
...
* 将副本编辑为直接指向点。
* 使用主动语态使句子结构更简单。
* 避免使用某些人可能不知道的俚语、行话或地区性用语。

要评估内容可读性，请使用Microsoft Word中常用的[Flesch阅读简易测试](https://support.microsoft.com/en-us/office/get-your-document-s-readability-and-level-statistics-85b4969e-e80a-4777-8dd3-f7fc3c8b3fd2){target="_blank"}。 它按0-100的数值计算内容的读取难易程度。

## 测试您的内容 {#test}

要验证内容可访问性，您可以使用[!DNL Journey Optimizer B2B Edition]提供的测试功能。 它们并非专门用于检查您的内容是否完全可访问，但它们可以提供第一级别的验证。

* 使用测试配置文件预览内容。

* 使用[呈现电子邮件](email-test-rendering.md)选项，该选项利用Litmus在主要电子邮件客户端(Apple Mail、Gmail、Outlook)间模拟您的设计，并查看文本、颜色和图像是否可以访问您的内容。

* 在将内容发送到实际受众之前，请发送校样以测试内容的渲染。

![具有测试配置文件预览选项的内容模拟界面](assets/accessible-simulate.png){width="800" zoomable="yes"}

要检查您的内容是否能够以更一致的方式可靠地访问，请使用外部工具，例如：
...
![深色模式的设计设置](assets/email-color-mode-dark-text-settings.png){width="700" zoomable="yes"}

对于深色模式设计，请使用透明的PNG或SVG图像，并设置相应的元标记和CSS。 如果不支持深色模式，请提供可访问的回退样式。 最后，在浅色和深色模式下测试所有内容和UI元素。

有关在电子邮件设计空间中使用&#x200B;_深色模式_&#x200B;的详细信息，请参阅电子邮件内容的&#x200B;[_深色模式_](./email-dark-mode.md)。

## 使用特定属性进行辅助功能 {#attributes}

HTML属性可帮助辅助型技术解释内容中的结构、语言和关系。 以下部分介绍了语言和文本方向、用于布局对比数据的表以及屏幕阅读器向用户公告的链接文本。
...
使用设计工具添加指向内容的链接并编辑标签，使其可见（可见）且具有描述性（清除用途）。 避免&#x200B;_此处_&#x200B;或&#x200B;_更多_&#x200B;之类的模糊标签。

![显示URL字段和描述性标签选项的链接设置面板](assets/accessible-link.png){width="700" zoomable="yes"}
