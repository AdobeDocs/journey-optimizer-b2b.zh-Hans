...
>title="为深色模式使用特定图像"
>abstract="您可以选择另一个图像，用于在启用深色模式时显示。 <br>添加深色模式的特定图像并不能保证其在所有电子邮件客户端中都能正确渲染。 请注意，并非所有电子邮件客户端都支持自定义深色模式。"
...
随着主要电子邮件客户端采用深色模式的次数增加，必须考虑您的电子邮件在浅色和深色环境中的呈现方式 — 无论您是否使用[自定义深色模式](#custom-dark-mode)。
