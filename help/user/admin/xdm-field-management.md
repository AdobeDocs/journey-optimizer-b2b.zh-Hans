...
体验数据模型(XDM)字段是向[!DNL Journey Optimizer B2B Edition]应用程序提供数据的架构元素。 在历程、购买组和功能（如电子邮件个性化和条件内容）中使用XDM字段作为筛选器和约束。
...
当您选择&#x200B;**[!UICONTROL 可更新字段]**&#x200B;时，_选择字段_&#x200B;对话框允许您从其他数据源中选择字段。

1. 对于&#x200B;**[!UICONTROL 数据集]**，请选择要配置的数据源。
1. 编辑所选数据集中的字段。
