...
系统创建一个名为&#x200B;_活动权重模型1_&#x200B;的初始参与度得分模型。 模型状态和参与活动取决于[!DNL Journey Optimizer B2B Edition]环境的数据架构：

* **简化的架构** (Beta) — 如果您的环境使用[简化的架构](../simplified-architecture.md)，则参与活动基于标准和自定义Experience Platform事件。 默认情况下，所有活动的权重为0。

  ![简化架构的默认参与度得分加权模型](./assets/configuration-engagement-scoring-model-default.png){width="600" zoomable="yes"}

* **标准架构** — 如果您的环境使用标准架构，则连接的[!DNL Marketo Engage]实例是参与活动数据的源。 默认模型处于活动状态，直到您创建自定义版本并将其激活为止。

  ![标准体系结构的默认参与度得分加权模型](./assets/configuration-engagement-scoring-model-default-me.png){width="600" zoomable="yes"}
...
简化架构的默认模型包括Experience Platform跟踪的活动。 每个活动的权重为零(0)（不使用），直到您为其分配权重为止。 所有活动的最大每日频率也是20，您不能对其进行更改。
