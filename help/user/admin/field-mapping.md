...
| `consents.marketing.email.val` | 已取消订阅 | 已取消订阅 | 字符串 | 如果取消订阅为true（例如，值= 1），则将`consents.marketing.email.val`设置为(n)。 如果取消订阅为false（例如，值= 0），则将`consents.marketing.email.val`设置为null。 |
