...
1. 通过输入您的Adobe ID电子邮件、用户名、名字或姓氏进行搜索。
   ...
   * 如果&#x200B;**[!UICONTROL 管理员角色]**&#x200B;列中的值显示`System`，则显示的用户是系统管理员。
...
>[!TIP]
>
>从旧版部署迁移并将现有用户添加到Marketo Engage时，您可以跳过用户组创建，只需打开现有组并添加Marketo Engage产品配置文件即可。
...
1. 添加每个用户：
   ...
   * 输入用户的电子邮件地址、名字和姓氏。
     ...
     ![Experience Platform — 为新角色添加用户配置文件](./assets/admin-console-add-users.png){width="600" zoomable="yes"}
...
权限是单一的权利，可用于定义分配给产品配置文件的授权。 每个权限都分组到一个功能下，例如&#x200B;_历程_&#x200B;或&#x200B;_购买组_。 这些功能表示Journey Optimizer B2B Edition中的函数或对象。
...
| B2B历程 | 配置、管理、查看和发布B2B历程的权限。 这些权限包括帐户和人员操作、事件侦听器以及拆分路径。 | <li>管理B2B历程 |
...
| B2B销售用户 | <li>查看B2B参与仪表板 <li>访问In-CRM Insights |

### 编辑角色权限

对于内置或自定义角色，您可以随时决定添加或删除权限。 如果修改默认或自定义角色，则会影响分配给该角色的每个用户。

在以下示例中，您要为分配给B2B历程管理员角色的用户添加与B2B角色资源相关的权限。 此更改允许该角色中的用户也管理帐户历程。
...
1. 单击&#x200B;**[!UICONTROL 添加群组]**。
   ...
   ![Experience Platform — 将组添加到角色](./assets/aep-permissions-role-add-groups.png){width="700" zoomable="yes"}
