...
Adobe Journey Optimizer B2B Edition构建于Adobe Experience Platform之上。 通过此实施，Journey Optimizer B2B Edition和Marketo Engage将不在同一系统和同一数据存储中。 Journey Optimizer B2B Edition从Adobe Experience Platform接收数据。 但是，它继续依赖Marketo Engage权利和一些后端功能（如电子邮件投放）来配置和配置系统。
