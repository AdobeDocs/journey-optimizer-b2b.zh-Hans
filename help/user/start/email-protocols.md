...
Adobe Journey Optimizer B2B edition利用附加的Marketo Engage实例中的电子邮件渠道功能和事件跟踪。 对于使用限制性防火墙或代理服务器设置的组织，为了确保电子邮件传递按预期工作，系统管理员必须将某些域和IP地址范围添加到允许列表中。
...
此外，为您的`FROM:`域配置特定于DMARC的DNS TXT记录。 您也可以选择定义一个电子邮件地址，由此指定 DMARC 报告在您组织内应发送到该地址，以便进行报告监控。
...
这些报告的主要用途是获得那些试图进行欺骗的电子邮件的概述情况。 它们是具有高度技术性的报告，最好通过第三方工具进行分析。
...
Marketo Engage代表您与Internet服务器进行出站连接。 您的IT部门和某些合作伙伴/供应商可能会使用允许列表来限制对服务器的访问。 如果是这样，您必须向他们提供Marketo Engage出站IP地址块，以添加到他们的允许列表。
