...
要支持由附加的Marketo Egage实例提供的电子邮件投放基础架构，请设置以下电子邮件选项。 Marketo Engage产品管理员可以通过导航到Marketo Engage实例中的&#x200B;**[!UICONTROL 管理员]**&#x200B;区域并选择&#x200B;**[!UICONTROL 电子邮件]**&#x200B;来配置这些设置。
...
电子邮件内容的显示功能有限（CSS有限，无JavaScript或表单）。 营销人员可以使用&#x200B;_以网页形式查看_&#x200B;选项通过Marketo Munchkin为电子邮件收件人应用Cookie。 作为产品管理员，您应该配置默认的HTML以及在营销人员选择此选项时填充的文本。
...
字段。
   ...
   支持10 - 100之间的值。 _[!UICONTROL 子检索限制]_&#x200B;是通过1000除以父限制自动设置的。 例如，如果将父限制设置为50，则子限制的计算公式为20 (1000 ÷ 50 = 20)。

1. 单击&#x200B;**[!UICONTROL 保存更改]**。

## 自定义标题选项

更改电子邮件的&#x200B;_[!UICONTROL 自定义标头选项]_&#x200B;以配置电子邮件跟踪链接标头。 启用这些选项以使用使用严格传输的HTTPS实施安全跟踪链接。

1. 转到附加的Marketo Engage实例中的&#x200B;**[!UICONTROL 管理员]**&#x200B;区域，然后选择&#x200B;**[!UICONTROL 电子邮件]**。

1. 滚动到&#x200B;_[!UICONTROL 自定义标题选项]_&#x200B;面板，并根据您的跟踪链接策略更改设置：

   ![Marketo Engage电子邮件管理员 — 自定义标题选项默认设置](./assets/me-admin-email-custom-object-retrieval-limits.png){width="500"}

   * **[!UICONTROL 严格传输安全性]** — 将此选项设置为“已启用”可保证跟踪链接始终通过HTTPS提供（只应针对跟踪链接受SSL保护的订阅进行设置）。
...
1. 切换滑块以启用一个或两个。
   ...
   >[!NOTE]
   >
   >将“活动之间的持续时间”_设置为`0`秒，Marketo Engage可识别在同一秒发生的电子邮件活动。_&#x200B;如果在指定的秒数内发生了多个电子邮件活动，则会将其识别为机器人活动。

   要禁用任一过滤方法，请将滑块切换到左侧。 如果这样做，数据将不会重置。

### IP阻止列表

Adobe已确定一个IP地址列表，这些地址负责生成数百万个虚假参与，例如，从以下任何IP收到的此类参与将被自动过滤掉，并且不会添加到您的Marketo Engage实例。 此筛选可能会导致电子邮件打开次数、点击次数和其他相关活动减少。 此列表可能会定期更新。
...
>[!NOTE]
>
>在将IP地址列入此列表之前，会仔细分析和审查每个IP地址，确保仅阻止最关键和最有害的IP。
