...
| 功能 | 电子邮件设计 — 支持Firefly和自定义创作AI模型 | 您现在可以启用标准和自定义Firefly模型以及批准的第三方图像模型（例如NanoBanana）的集成。 营销人员可以针对每个用例选择最佳模型：可满足一般需求的标准Firefly、用于品牌内生成的自定义Firefly，或针对专用或实验场景的批准的第三方模型。 |
| 增强功能 | 电子邮件设计 — 内容质量验证 | 除了品牌协调之外，您还可以评估总体内容质量，以发现可读性、一致性和有效性方面的潜在问题（独立于您的品牌指南）。 这些自动化检查有助于识别消息表述不清、语调不一致或结构性差距问题。 |
| 增强功能 | 帐户和人员历程 — 支持人员自定义对象 | (Beta)利用链接到帐户的关系数据来过滤帐户或人员历程中的人员。 [了解详情](../journeys/split-merge-paths-nodes.md#custom-data-filtering) |
| 增强功能 | (Beta)内容个性化 — 支持人员自定义对象 | 在使用自定义对象定义内容个性化时，您可以访问基于模型的类自定义对象（关系架构）的变量。 [了解详情](../content/personalization.md#custom-datasets) |
| 增强功能 | 激活到目标 — 可重复使用的受众 | 您现在可以在&#x200B;_激活到同一历程中的目标_&#x200B;历程操作中重用虚拟受众，并从虚拟受众中删除帐户。 |
...
| 增强功能 | 购买组成员特殊筛选器 — 已移除限制 | 购买组&#x200B;_[!UICONTROL 的]_&#x200B;成员现在包括&#x200B;_已移除_&#x200B;限制。 将此限制添加到筛选器时，您可以包括已删除的购买组成员，也可以排除他们。 Marketo Engage智能列表中也支持此功能，您可以在其中的&#x200B;_[!UICONTROL 购买组成员]_&#x200B;筛选器中使用此新限制。 |
