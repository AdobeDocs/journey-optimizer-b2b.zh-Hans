...
>[!NOTE]
>
>这些版本更改从2026年8月14日开始部署，并分阶段推出每个功能和增强功能。 功能及增强功能的发布时间可能会有变动。

## 2026.6发行说明 {#rel-2026-6}

**部署日期**：2026年7月10日

| 类型 | 项目 | 描述 |
| ---- | ---- | ----------- |
| 功能 | _变量拆分路径_&#x200B;历程节点 — 帐户历程 | (Beta)营销人员现在可以通过根据定义的百分比将帐户或人员分配给不同的旅程路径，测试旅程中的变体。 [了解详情](../journeys/variant-split-paths-nodes.md) |
| 增强功能 | _侦听事件_&#x200B;历程节点 | 在个人历程中，事件侦听器节点现在可以配置为触发多个事件。 第一个符合条件事件在历程中向前推进记录。 （在客户历程中为即将发布的版本提供计划支持。） [了解详情](../journeys/listen-for-event-nodes.md#person-journeys) |
| 增强功能 | _侦听事件_&#x200B;历程节点 | 人员/人员事件的&#x200B;_[!UICONTROL 筛选器]_&#x200B;选项卡已从单个操作选择器更改为规则生成器界面。 这可以使用配置的体验事件支持多个活动配置。 [了解详情](../journeys/listen-for-event-nodes.md#filters-people-event) |
| 增强功能 | _执行操作_&#x200B;历程节点 | 帐户历程和人员历程的人员上下文现在有其他操作可用：_添加到人员历程_&#x200B;和&#x200B;_从人员历程中删除_。 [了解详情](../journeys/action-nodes.md#people-actions) |

>[!NOTE]
>
>这些版本更改从2026年7月10日开始部署，并分阶段推出每个功能和增强功能。 功能及增强功能的发布时间可能会有变动。

## 2026.5发行说明 {#rel-2026-5}
...
>[!NOTE]
>
>这些版本更改从2026年5月22日开始部署，并分阶段推出每个功能和增强功能。 功能及增强功能的发布时间可能会有变动。
...
## 2026.3发行说明 {#rel-2026-3}

**部署日期**：2026年3月27日

| 类型 | 项目 | 描述 |
| ---- | ---- | ----------- |
| 功能 | WhatsApp 渠道 | 营销人员现在可以通过[!DNL Meta Cloud API]直接从帐户历程发送WhatsApp消息。 此功能可实现WhatsApp消息传送的无缝集成，以支持历程内容渠道。 [了解详情](../admin/configure-channels-whatsapp.md) |
| 功能 | 支持[!DNL Firefly]和自定义生成AI模型 | 营销团队现在可以启用标准和自定义[!DNL Firefly]模型以及批准的第三方图像模型（如[!DNL NanoBanana]）的集成。 电子邮件设计人员可以针对每个用例选择最佳模型：可满足一般需求的标准[!DNL Firefly]、用于品牌内生成的自定义[!DNL Firefly]，或针对专用或实验场景的已批准的第三方模型。 [了解详情](../content/generative-ai-models.md) |
| 功能 | 历程的自定义外部操作 | 开发人员现在可以使用API构建与其第一方系统的集成。 通过这些自定义集成，营销人员可以添加&#x200B;_外部操作_&#x200B;和&#x200B;_外部拆分路径_&#x200B;节点，以在帐户历程执行期间向外部服务发出出站请求。 [了解详情](../admin/configure-external-actions.md) |
| 功能 | 品牌 | (Beta)营销团队可以通过存储和管理品牌配置文件来维护电子邮件内容资产中的品牌一致性。 通过添加资产（如颜色、字体、徽标、主题、可视内容和合规性指南），他们可以使用品牌配置文件来创建创作AI内容。 他们还可以衡量品牌一致性以确保法规遵从性。 [了解详情](../content/brands-overview.md) |
| 增强功能 | 发送电子邮件 — 优化发送时间 | 对于个人历程中的&#x200B;_发送电子邮件_&#x200B;操作节点，您可以使用&#x200B;_发送时间优化_&#x200B;选项，通过预测每个用户档案最有可能参与的时间，个性化电子邮件投放时间。 [了解详情](../content/email-send-time-optimization.md) |
| 增强功能 | 电子邮件模板设计工具 — 高级HTML模式 | 在电子邮件模板设计空间中使用&#x200B;_高级HTML模式_，用户可以进行细微的HTML/CSS编辑并将脚本标记添加到电子邮件模板以解决渲染问题。 [了解详情](../content/email-template-advanced-html.md) |
| 增强功能 | 人员自定义对象 — 购买群组角色模板 | 当管理员配置与业务人员配置文件相关的自定义对象时，营销人员现在可以使用这些自定义对象定义购买群组角色。 [了解详情](../buying-groups/buying-groups-role-templates.md#add-the-template-roles) |
| 增强功能 | 电子邮件内容评分 — 内容质量评估 | 除了品牌协调之外，您还可以评估总体内容质量，以发现可读性、一致性和有效性方面的潜在问题（独立于您的品牌指南）。 这些自动化检查有助于识别消息表述不清、语调不一致或结构性差距问题。 [了解详情](../content/content-evaluation.md) |

>[!NOTE]
>
>这些版本更改从2026年3月27日开始部署，并分阶段推出每个功能和增强功能。 功能及增强功能的发布时间可能会有变动。


## 2026.2发行说明 {#rel-2026-2}

**部署日期**：2026年2月20日

| 类型 | 项目 | 描述 |
| ---- | ---- | ----------- |
| 功能 | XDM字段/关系架构 — 支持人员自定义对象 | (Beta)现在，管理员可以使用与帐户的单级别一对一关系选择与人员相关的自定义对象。 此功能使您的营销组织能够呈现更丰富的真实业务数据视图，从而定向、个性化和报告人员或帐户级别以外的实体。 [了解详情](../admin/xdm-field-management.md#relational-schemas) |
| 功能 | 历程重新进入 | 您现在可以通过历程工作流多次发送帐户/人员。 重返工作涉及多种情形，例如重新评估资格标准和可重复使用的培养工作流程。 [了解详情](../journeys/journey-re-entry.md) |
| 增强功能 | 帐户和人员历程 — 支持人员自定义对象 | (Beta)利用链接到帐户的关系数据来过滤帐户或人员历程中的人员。 [了解详情](../journeys/split-merge-paths-nodes.md#custom-data-filtering) |
| 增强功能 | (Beta)内容个性化 — 支持人员自定义对象 | 当营销人员使用自定义对象定义内容个性化时，他们可以访问基于模型的类自定义对象（关系架构）的变量。 [了解详情](../content/personalization.md#custom-datasets) |

>[!NOTE]
>
>这些版本更改从2026年2月20日开始部署，并分阶段推出每个功能和增强功能。 功能及增强功能的发布时间可能会有变动。

## 2026.1发行说明 {#rel-2026-1}
...
| 功能 | 适用于 Salesforce 和 Dynamics 的销售洞察 | 销售团队成员现在可在 Salesforce 或 Dynamics 集成中查看正在成熟的购买群体及相关洞察，从而发现新的销售机会。 系统会显示购买群体的阶段、评分及相关成员等详细信息。 [了解详情](../buying-groups/incrm-insights.md) |
...
* **专为长期可扩展性而设计**：此更新使您的组织为持续改进和创新做好准备。

>[!NOTE]
>
>查看[设置核对清单](../setup-ultimate.md)以了解配置指导。

2025.10版本中提供了以下新增功能和增强功能，以及更新的架构：
...
| 类型 | 项目 | 描述 |
| ---- | ---- | ----------- |
| 功能 | 与 GenStudio for Performance Marketing 集成 | （有限提供）您现在可以将 GenStudio for Performance Marketing 电子邮件体验与 Journey Optimizer B2B Edition 集成，以提高营销效率并保持品牌一致性。 通过这种集成，您可以将 GenStudio AI 驱动的内容创建与 Journey Optimizer B2B Edition 中的高级编排功能结合起来。 [了解详情](../content/genstudio-email-workflow.md) |
...
| 功能 | 帐户和购买群组行动 | 直接在帐户详细信息和购买群组详细信息页面上采取操作，以实现及时且有针对性的参与。 <li>使用&#x200B;_发送电子邮件_&#x200B;操作，将已批准的 Marketo Engage 电子邮件发送给选定的帐户联系人或购买群组成员。 [了解详情](../accounts/account-details.md#send-email) <li>在购买群组详细信息中的行动还包括&#x200B;_分配新成员_、_删除成员_&#x200B;和&#x200B;_编辑角色_。 [了解详情](../buying-groups/buying-group-details.md#members-tab) |
...
| 增强功能 | 电子邮件的 Handlebar 令牌格式 | 电子邮件内容的个性化令牌现在使用一种更新格式，与 Handlebar 脚本完全兼容。 此格式使用&#x200B;_驼峰式大小写_&#x200B;或下划线，不使用空格。 [了解详情](../content/email-authoring.md#personalize-content) |
| 增强功能 | 列表的总数显示 | 改进了&#x200B;_[!UICONTROL 解决方案兴趣]_&#x200B;和&#x200B;_[!UICONTROL 帐户历程]_&#x200B;两个列表页面，在搜索栏旁边显示总数。 |

## 2025.4 版发行说明 {#rel-2025-4}
...
| 类型 | 项目 | 描述 |
| ---- | ---- | ----------- |
| 功能 | 重复的帐户历程 | 现在为帐户历程提供重复操作。 您可以复制帐户历程的详细信息，或者仅复制流程和路径结构的简单框架。 <a href="../journeys/journeys-overview.md#duplicate-journey">了解详情</a> |
| 功能 | 我的帐户历程令牌 | 您现在可以定义一组其值特定于帐户历程的自定义令牌。 这组自定义令牌被称为&#x200B;_我的令牌_，所有这些自定义令牌都可以在创作历程电子邮件时用于进行个性化。 <a href="../content/personalization-my-tokens.md">了解详情</a> |
...
| 功能 | 历程复制类型 | 当您复制帐户历程时，您可以包含节点详细信息，其中不包括 Journey Optimizer B2B Edition 中创建的电子邮件和 SMS 消息。 或者，您可以创建结构和路径流的精简副本，而不包含节点详细信息和设置。 <a href="../journeys/journeys-overview.md#duplicate-journey">了解详情</a> |
...
| 功能 | 付费媒体目标 | 通过帐户历程为付费媒体营销活动筛选已知人员，以便您可以在领英等广告平台上进一步吸引他们。 根据特定行为，使用拆分路径节点区段帐户受众，并确定承诺额外参与的帐户。 然后，通过 Real-Time CDP 将这些帐户中的人员添加到外部客户受众中，并将其添加到受支持的付费媒体目标中。 <a href="../journeys/action-nodes.md#journey-optimizer-b2b-actions">了解详情</a> |
| 功能 | 智能仪表板 | 查看购买群组在帐户历程中的进展情况，包括 AI 生成的洞察，以便进行更智能的分析和准确的帐户优先级排序。 <a href="../dashboards/intelligent-dashboard.md">了解详情</a> |
| 功能 | 购买群组和帐户详细信息 | 查看购买群组和帐户级别的洞察，以便在开始吸引客户时掌握更多的背景信息和历史数据。<p>购买群组详细信息包括识别到的任何第一方意图。 <a href="../buying-groups/buying-group-details.md">了解详情</a><p>帐户详细信息页面突出显示检测到的参与意向激增，以便营销人员能够提醒销售人员哪些帐户已准备好进行以自定义销售为中心的参与。  <a href="../accounts/account-details.md">了解详情</a> |
| 功能 | 历程概述仪表板 | 对于帐户历程，**[!UICONTROL 概述]**&#x200B;选项卡提供活动历程的全面快照，其中包含圆圈和条形图，可对完成和参与活动进行分类和量化。 <a href="../dashboards/journeys-dashboard.md">了解详情</a> |
| 功能 | Adobe Express 图像编辑 | Adobe Express 快速操作允许您对图像进行简单的编辑（例如裁剪和调整大小），以使内容看起来更加精美。 <a href="../content/image-edit-adobe-express.md#quick-actions-in-adobe-express">了解详情</a>  <p>为了提供更全面的设计工具，此集成可在 Journey Optimizer B2B Edition 中启用完整的 Adobe Express 许可证。 通过此设置，可在本地资产工作区内访问完整的 Adobe Express 用户界面。 <a href="../content/image-edit-adobe-express.md#adobe-express-enterprise-license">了解详情</a> |
...
| 增强功能 | Marketo Engage 智能列表的购买群组过滤器 | 在 Marketo Engage 中查看并创建包含购买群组过滤器的智能列表。 通过这些添加的过滤器，您可以从 Journey Optimizer B2B Edition 帐户历程中抑制或包含 Marketo Engage 营销活动和计划中的购买群组成员。 _已弃用功能_ |
| 增强功能 | 用于历程和角色的 Marketo Engage 列表成员资格过滤器 | 在 Journey Optimizer B2B 中，检查 Marketo Engage 列表会员资格作为&#x200B;_按人员拆分路径_&#x200B;节点的条件，以帮助消除历程活动中的重复。 <a href="../journeys/split-merge-paths-nodes.md#add-a-split-path-by-people-node">了解详情</a> <p> 为购买群组角色模板使用列表会员资格作为角色条件。 <a href="../buying-groups/buying-groups-role-templates.md#add-the-template-roles">了解详情</a> |
| 增强功能 | 参与度概述仪表板 | 此仪表板已更新，提供全面的参与度视图。 它通过快照圆形图和显示一段时间内趋势的线形图，展示帐户和个人互动的实时量度。 <a href="../dashboards/engagement-dashboard.md">了解详情</a> |

## 2024 版 {#2024-releases}
...
| 功能 | 最近访问的资产 | Journey Optimizer B2B Edition 应用程序中的主页现在包含&#x200B;_[!UICONTROL 最近访问的]_&#x200B;部分，该部分为营销人员或管理员提供了最近访问的资产列表。 您可以使用此列表直接前往您最近使用的资产，而无需浏览一系列资产页面进行搜索。 <p>该列表提供了有关修改的额外信息，以便您决定自上次会话以来哪些资产需要进一步修改。 对于电子邮件资产，它会显示使用该电子邮件资产的帐户历程。 <a href="../home-page.md">了解详情</a> |
...
| 增强功能 | 历程拆分节点——其他活动历史记录条件属性 | 在使用条件定义按人员拆分节点的路径筛选时，有两个额外的属性：_打开的电子邮件_&#x200B;和&#x200B;_是已送达的电子邮件_。 这些额外属性为历程中根据电子邮件活动筛选的人员提供了更大的灵活性。 <a href="../journeys/split-merge-paths-nodes.md#split-paths-by-accounts">了解详情</a> |
