---
title: WhatsApp渠道配置
description: 通过Meta的Cloud API连接您的WhatsApp Business帐户，以便在Journey Optimizer B2B Prime人员历程中启用WhatsApp消息传送。
...
Journey Optimizer B2B Prime通过Meta的Cloud API发送WhatsApp消息。 在营销人员创建人员历程的WhatsApp消息之前，产品管理员必须配置WhatsApp渠道。

适用于Journey Optimizer B2B Prime的![WhatsApp任务流](../../user/admin/assets/whatsapp-flow-diagram.png)
...
>[!IMPORTANT]
>
>您对WhatsApp消息传递服务的使用受Meta条款和条件的约束。 通过Journey Optimizer B2B Prime访问WhatsApp消息传送，即表示您确认已查看并同意遵守[Meta WhatsApp业务政策](https://www.whatsapp.com/legal/business-policy/)。

## 限制 {#limitations}

以下限制适用于 WhatsApp 渠道：

* Adobe Journey Optimizer B2B Prime **不符合HIPAA要求，并且不符合HIPAA要求**。 此外，第三方供应商不受Adobe BAA的约束。 客户需对合规性和供应商验证自行负责。
...
在发送WhatsApp消息之前，必须配置Journey Optimizer B2B Prime环境，并将其与您的WhatsApp帐户连接。
...
1. 配置API凭据，如下所述：
   ...
   * **[!UICONTROL 业务帐户ID]** — 输入与业务组合相关的唯一编号。 有关信息，请参阅[Meta文档](https://www.facebook.com/business/help/1181250022022158?id=180505742745347)。
   ...
   ![WhatsApp设置API凭据](../../user/admin/assets/config-whatsapp-channels-api-credentials.png){width="500" zoomable="yes"}

1. 单击&#x200B;**[!UICONTROL 继续]**。

1. 选择要连接到WhatsApp API凭据的&#x200B;**[!UICONTROL WhatsApp商业帐户]**。

   ![WhatsApp设置API凭据选择业务帐户](../../user/admin/assets/config-whatsapp-channels-api-credentials-details.png){width="500" zoomable="yes"}
...
1. 外部测试凭据 — 直接使用Meta API验证您的凭据，以确认问题与凭据有关，还是与Journey Optimizer B2B Prime凭据处理有关。
...
Webhook使Journey Optimizer B2B Prime能够从WhatsApp商业帐户接收入站消息、同意响应和投放通知。 配置Webhook以确保正确的同意管理和消息跟踪。
...
1. 输入&#x200B;**[!UICONTROL 回复邮件]**，当收到的邮件与此类别中的关键字匹配时，自动发送。
   ...
   ![包含选择加入关键字和回复消息的WhatsApp设置Webhook](../../user/admin/assets/config-whatsapp-channels-webhooks.png){width="500" zoomable="yes"}
...
1. 复制&#x200B;**[!UICONTROL 验证令牌]**&#x200B;和&#x200B;**[!UICONTROL Webhook URL]**&#x200B;值。
   ...
   ![用于复制URL和验证令牌的WhatsApp设置webhook设置](../../user/admin/assets/config-whatsapp-channels-webhooks-copy-token-url.png){width="500" zoomable="yes"}
...
1. 输入用于邮件传递的&#x200B;**[!UICONTROL 发件人电话号码]**。
   ...
   ![WhatsApp渠道配置详细信息](../../user/admin/assets/config-whatsapp-channels-general-create.png){width="500" zoomable="yes"}

1. （当前不适用于Journey Optimizer B2B Prime）对于&#x200B;**[!UICONTROL WhatsApp执行字段]**，请选择在收件人有多个电话号码可用时用作优先电话号码的配置文件属性。
